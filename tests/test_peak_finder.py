...
class TestValidateArgs:
    def test_multidimensional_array_raises_error(self):
        x: npt.NDArray[float] = np.asarray([[1, 2], [3, 4]], dtype=float)
    ...
    def test_negative_nups_raises_error(self):
        x: npt.NDArray[float] = np.asarray([1.0, 2.0, 3.0], dtype=float)
...
class TestEncodeSigns:
    def test_increasing_sequence(self):
        x: npt.NDArray[float] = np.asarray([1.0, 2.0, 3.0, 4.0], dtype=float)
        result: str = _encode_signs(x, "0")
        assert result == "+++"

    def test_decreasing_sequence(self):
        x: npt.NDArray[float] = np.asarray([4.0, 3.0, 2.0, 1.0], dtype=float)
        result: str = _encode_signs(x, "0")
        assert result == "---"

    def test_flat_sequence_with_zero(self):
        x: npt.NDArray[float] = np.asarray([1.0, 1.0, 1.0], dtype=float)
        result: str = _encode_signs(x, "0")
        assert result == "00"
    ...
    def test_mixed_sequence(self):
        x: npt.NDArray[float] = np.asarray([1.0, 2.0, 2.0, 3.0, 2.0], dtype=float)
        result: str = _encode_signs(x, "0")
        assert result == "+0+-"
...
class TestEnforceMinimumPeakDistance:
    ...
    def get_df() -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {
                "height": np.asarray([10.0, 8.0, 5.0], dtype=float),
                "peak_idx": np.asarray([0, 2, 10], dtype=int),
                "start_idx": np.asarray([0, 1, 9], dtype=int),
                "end_idx": np.asarray([1, 3, 11], dtype=int),
            }
        )
...
class TestFindPeaks:
    ...
    def test_accepts_numpy_array(self):
        x: npt.NDArray[float] = np.asarray([0.0, 1.0, 2.0, 1.0, 0.0])
        result: pd.DataFrame = find_peaks(x, min_peak_height=0.0)
        assert len(result) == 1
