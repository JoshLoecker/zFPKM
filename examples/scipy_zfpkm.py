...
try:
    ...
    logger.warning("Install scipy first! python3 -m pip install scipy")
...
def scipy_zfpkm():
    ...
    for i, col in enumerate(fpkm.columns):
        ...
        kde = gaussian_kde(log2_col)

        x_range: npt.NDArray[float] = np.linspace(log2_col.min(), log2_col.max(), 512)
        density: npt.NDArray[float] = kde(x_range)
        peaks, peak_parameters = find_peaks(density, height=0.02, distance=1.0)
        ...
        zfpkm_results.append(ZFPKMResult(name=col, density=DensityResult(x=x_range, y=density, bw=1.0, n=512), mu=mu, sd=stddev, fpkm_at_mu=max_fpkm))
